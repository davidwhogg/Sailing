...
% - Should Terry Tao be cited way earlier?
...
% - Should we re-write the planar force to not require that the unit vector be pointed down-wind? YES WE SHUOLD.
% - Overleaf seems to have wrong papersize? See papersize/geometry hacks.
...
The Galilean-relativistic thing to say is that the boat is powered by the relative velocity between the air and the water (the symmetry between the air and the water in the mechanisms of sailing is the fundamental theme of \cite{symmetry}).
...
This \documentname{} is inspired by these observations.
...
The only prior work we know that does precisely this is \cite{tao}, but that model is very simplistic (for example, it doesn't lead to steady-state sailing).
