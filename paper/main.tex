% This paper is copyright 2021, 2025 the authors.
...
% - Do we capitalize north and east and southeast>

% to-do items:
% ------------
% - THE RATIO OF WATER TO AIR IS 800 NOT 700. Dude
% - Hogg believes that there is a relationship between the lift ratios and the best-sailing angle between sail and keel.
% - Incorporate references and terminology from Leif Ristroph (NYU). See email of 2025-01-24 and 25.
% - Update figure captions to match new figures!
% - Add to the abstract and the introduction and discussion that this is only a 2-D model of sailing. Not 3-D!
...
% - Make sure the spirit of the bibliographic comments get pulled into the main text (and removed from the bibiliography).
% - Fix the et al typography in the bibliography?
...
% - What claims in the literature exist about maximum boat speeds? Have I seen a 2x wind claim? We can beat that. Have I seen a "never faster downwind than the wind" (as in Kimball) or than 0.5 times the wind claim? We can perhaps beat that too.
% - Should we be more careful to use Galilean-relativistic language, like instead of "upwind" say "upwind relative to the water"? Like always?
% - Mention sailing hydrofoil where we discuss the keel lift ratio: The point of the hyrdrofoil is to reduce A_below.
% - Explain why amateur sailboats have huge keels and professional boats have small keels. Upwind sailing is the answer!
% - Kleban points about non-trivial wind force f(theta) and references (in email, maybe).
% - Note that better sailing (tm) is almost never in the direction of the target!
% - Is there an analog of the Betts or Betz limit? Also just generally discussion of power, power dissipated, and so on.
% - Do we adequately mention sustainability in the introduction?
% - How to plot the phase space of dimensionless ratios that allow for UW and DWFTTW?
% - Can we address stability and uniqueness of the solutions to the (quartic, actually) equations?
% - Do we clearly define the dimensionless ratios that define a boat? There are three.
% - Kleban wants polars -- plots that are used in the business. Can do!
% - Discuss "velocity made good" in the text and maybe name-check it in the abstract?
% - Check stability.  We decided to start from steady sailing, change v_boat by Delta v_boat, and then verify that the acceleration is of the form \vec a = S Delta v, where S is a matrix with two negative eigenvalues.

\documentclass[letterpaper]{article}
\usepackage[utf8]{inputenc}
\usepackage[letterpaper]{geometry}
\PassOptionsToPackage{hyphens}{url} % huh?
...
\title{\bfseries%
The spherical scow:\\
A simple model for sailing}
\markright{\textsf{Hogg \& Kleban / A momentum-transport model for sailing}}
\author{DWH, MK, others?}
\date{2021 through 2025 hahaha}
\begin{document}
\maketitle
