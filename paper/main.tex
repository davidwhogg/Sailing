...
HOGG: Comments on GoodSailing(tm): As we saw in \secref{sec:steady}, you can sail dwfttw, and you can sail upwind, and you can sail fttw in general. But you can't sail dufttw or ddfttw. The settings of the sail look ``tight'' relative to standard sailing practice. But this boat has flat sails, so it's hard to compare the exact settings with sailor experiences (which are all with curved sails).

HOGG: Final comment: But this is not the BEST a sailor can do, because (a) boat doesn't move in the $\uvec_{\parallel\keel}$ direction, (b) because the destination might also be moving wrt the water, and (c) because you might want to take a tacking path...
