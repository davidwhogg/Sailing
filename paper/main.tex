...
% - What claims in the literature exist about maximum boat speeds? Have I seen a 2x wind claim? We can beat that. Have I seen a "never faster downwind than the wind" or than 0.5 times the wind claim? We can beat that too.
...
Of course it's not consistent with Galilean relativity\footnote{We refer to Galilean relativity repeatedly in this \documentname. Of course Galilean relativity is just the low-velocity approximation to special relativity \cite{sr}. But we are not going to consider air--water velocity differences that are an appreciable fraction of the speed of light (except maybe in the discussion in \secref{sec:discussion}).} to say that the boat is powered by the wind:
...
and $\uvec_\perp^\top\vec{v}$ is the scalar inner product (dot product) of the two vectors.
MK - I'M NOT A FAN OF THIS NOTATION FOR INNER PRODUCT, ESPECIALLY WITH UPPER AND LOWER Ts.
...
HOGG NEED WE SAY MORE ABOUT THESE? PROBABLY YES THERE ARE COMMENTS TO BE MADE. LIKE FOR INSTANCE LIFT RATIOS.
...
The ratio of effective area of sail to hull-above is 250, as is the ratio of effective area of keel to hull-below.
...
\begin{figure}[t!]
  ...
  \caption{Each panel is analagous to \figref{fig:hourgood} but in each case we have varied one of the dimensionless ratios....HOGG....for varying values of the three ratios $A_\sail/A_\keel$, $A_\sail/A_\above$, and $A_\keel/A_\below$.\label{fig:design}}
  \figurerule
\end{figure}

HOGG: Comment on what we seen in \figref{fig:design}. HOGG: Note that if you set $\rho_\air\,A_\sail=\rho_\water\,A_\keel$ you get the fastest possible cross-wind sailing speed. Is that a result, and is it known?
...
HOGG: Take some time to bash the Bernoulli bullshit. Note also that the different-path-length part of that argument is explicitly not true for a sail!
